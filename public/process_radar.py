...
def fetch_and_compile_latest_radar():
    ...
    # Targeting KHGX (Houston, TX) as our anchor station
    station = 'KHGX'
    
    # Look back 15 minutes to guarantee NOAA has finalized writing the binary file to Amazon's disc
    ...
    # Grab the absolute newest operational file chunk from the array
    ...
    # Isolate lowest operational baseline base reflectivity sweep tilt
    ...
    # Downsample loop by steps of 2 to guarantee processing finishes well within free tier limits
    for r in range(0, num_radials - 1, 2):
        for g in range(0, num_gates - 1, 2):
            val = ref_field[r, g]
            
            # Filter out the noise floor (Ground clutter, clouds, birds under 20 dBZ)
